...
do
 componentId=$( cat "${componentFile}" | sed -e 's/bns://' | xmllint --xpath "string(/Component/@componentId)" - )
 componentName=$( cat "${componentFile}" | sed -e 's/bns://' | xmllint --xpath "string(/Component/@name)" - )
 componentVersion=$( cat "${componentFile}" | sed -e 's/bns://' | xmllint --xpath "string(/Component/@version)" - )
 componentType=$( cat "${componentFile}" | sed -e 's/bns://' | xmllint --xpath "string(Component/@type)" - ) 
 for (( i=1; i<=${rules}; i++ ))
  do 
   xpath="`cat "${sonarRulesFile}" | xmllint -xpath 'profile/rules/rule['$i']/parameters/parameter/key[text()="expression"]/../value/text()' -`"
   fail=`cat "${componentFile}" | sed -e 's/bns://' | xmllint --xpath "$xpath" - 2> /dev/null | wc -c`
   if [ "${fail}" -gt 0 ];  then
