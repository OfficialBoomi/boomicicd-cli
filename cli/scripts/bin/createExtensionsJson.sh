...
f_addConnection=""
index_c=$(echo ${json} | jq '.connections.connection | length')
...
f_addProperty=""
index_p=$(echo ${json} | jq '.properties.property | length')
...
do
	...
	propertyExists=$(echo "${extensionJson}" | jq --arg name ${property} '.properties.property[] | select(.name == $name)')
  ...
  then
	  ...
	  json=$(echo ${json} | jq --arg name ${property} --arg property_arg ${l} '.properties.property[$property_arg|tonumber]  |= . + {"@type": "", "name": $name, "value": ""}')
		l=$(( l + 1 ))
		f_addProperty="true"
	fi
done
